...
function initCategoryBanners(categories) {
  ...
  bannerCats.forEach(cat => {
    ...
    content.innerHTML = `
      ...
      <a href="#products" class="cat-banner-btn" onclick="filterByCategoryBanner('${cat.name}');return false;">
...
function initNavCatDropdown(categories) {
  ...
  menu.innerHTML = categories.map(c =>
    `<a href="#products" onclick="filterByCategoryBanner('${c.name}');document.getElementById('nav-cat-menu').classList.remove('open');return false;">
